...
def quant2list(quantFile, algorithm):
	...
	column = 3 if algorithm == 'kallisto' else 4
		...
		if len(lineage[key]) == 8:
			...
			for i in range(0, len(ranks)):
				...
				if ranks[i] not in ranks_Found:
					if ranks[i] == 'species' and lineage[key][0].split('|')[0] != 'no rank':
						lineage[key].insert(0, empty)
					elif ranks[i] == 'species' and lineage[key][0].split('|')[0] == 'no rank':
						lineage[key].insert(1, empty)
					elif ranks[i] == 'genus' and lineage[key][0].split('|')[0] != 'no rank':
						lineage[key].insert(1, empty)
					elif ranks[i] == 'genus' and lineage[key][0].split('|')[0] == 'no rank':
						lineage[key].insert(2, empty)
					elif ranks[i] == 'family' and lineage[key][0].split('|')[0] != 'no rank':
						lineage[key].insert(2, empty)
					elif ranks[i] == 'family' and lineage[key][0].split('|')[0] == 'no rank':
						lineage[key].insert(3, empty)
					elif ranks[i] == 'order' and lineage[key][0].split('|')[0] != 'no rank':
						lineage[key].insert(3, empty)
					elif ranks[i] == 'order' and lineage[key][0].split('|')[0] == 'no rank':
						lineage[key].insert(4, empty)
					elif ranks[i] == 'class' and lineage[key][0].split('|')[0] != 'no rank':
						lineage[key].insert(4, empty)
					elif ranks[i] == 'class' and lineage[key][0].split('|')[0] == 'no rank':
						lineage[key].insert(5, empty)
					elif ranks[i] == 'phylum' and lineage[key][0].split('|')[0] != 'no rank':
						lineage[key].insert(5, empty)
					elif ranks[i] == 'phylum' and lineage[key][0].split('|')[0] == 'no rank':
						lineage[key].insert(6, empty)
					elif ranks[i] == 'superkingdom' and lineage[key][0].split('|')[0] != 'no rank':
						lineage[key].insert(6, empty)
					elif ranks[i] == 'superkingdom' and lineage[key][0].split('|')[0] == 'no rank':
...
def buildTaxonomy(nodesFile, namesFile, taxid_Accessions):
	...
	for key in taxid_Accessions:
		...
		lineage[key] = tempLineage
	copy_dict = copy.deepcopy(lineage)
	for key in lineage:
		for index in range(0, len(lineage[key])):
			if lineage[key][index].split('|')[0] not in ranks:
				copy_dict[key].remove(lineage[key][index])
		if lineage[key][0].split('|')[0] == 'no rank':
			copy_dict[key].insert(0, lineage[key][0])
	lineage = copy.deepcopy(copy_dict)
	print("buildTaxonomy-before\n")
	for key in lineage:
		print(key, lineage[key])
	print("\n\n\n")
...
def counts2lineage(accession_Readcounts, taxid_Accessions, lineage):
	...
	print('counts2lineage')
	for i in lineage_Readcounts:
		print(i)
	print('\n\n\n')
...
if __name__ == '__main__':
	...
	parser.add_argument('-atFile', '-at', help = 'Accessions - TaxIDs file', default = None)
	parser.add_argument('-algorithm', '-al', help = 'Algorithm, Salmon or Kallisto or Cedar', default = 'salmon')
	...
	_taxid_Accessions, _notFound = tempaccessions2taxid(_accession_Readcounts, args.atFile)
